package cn.cibnmp.ott.ui.header;
 ...
 */
@SuppressLint("NewApi")
public class StickyListHeadersListView extends ListView implements AbsListView.OnScrollListener, View.OnClickListener, Pullable {

    private PullToRefreshLayout ptrl;

    public void setPullLoyout(PullToRefreshLayout ptrl) {
        this.ptrl = ptrl;
    }
...
//        }
        ...
        // 获取自定义属性和默认值
        mRightViewWidth = DisplayUtils.getValue((int) mTypedArray.getDimension(
    ...
    protected void onAttachedToWindow() {
        ...
        frame.setOnCanPullListener(new StickyListHeadersListViewWrapper.canPullListener() {
            @Override
            public boolean onCanPull(int data) {
                if (data == 0) {
                    return canPullDown();
                } else {
                    return canPullUp();
...
//	}
    ...
    private boolean isCalledFromSuper() {
        ...
        return callingFrame.getClassName().contains("android.widget.AbsListView") || callingFrame.getClassName().contains("android.widget.ListView") || callingFrame.getClassName().contains("android.widget.FastScroller");
    }
    ...
    public boolean canPullDown() {
        if (adapter.getToView() != null && adapter.toPosition == 0 && adapter.getToView().getTop() >= 0) {
            ptrl.releasePull();
            return true;
        }
        return true;
    ...
    public boolean canPullUp() {
        if (adapter.getToView() != null && adapter.getToPosition() && getChildAt(adapter.getCount() - 1) == null && adapter.getToView().getBottom() == getMeasuredHeight()) {
            ptrl.releasePull();
            return true;
        }
        return true;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mCurrentItemView != null) {
            TextView textView = (TextView) mCurrentItemView.findViewById(R.id.tagText);
