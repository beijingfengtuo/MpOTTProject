...
/**
 * Created by axl on 2018/1/19.
 */

public class DetailAdapter extends Adapter<RecyclerView.ViewHolder> {


    private final static String TAG = "HoldersAdapter";
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        try {
            ...
            switch (type) {
                ...
                //(标题栏)
                case HomeOneViewType.title_viewType:
                   DetailHolderHelper.ViewHolderHelper0(
                            position, (TitleViewHolder) holder, infoItemBean);
                    break;

                //(4张大图轮播)
                case HomeOneViewType.viewpager_viewType:
                    viewpagerholder = (ViewPagerViewHolder) holder;
                    DetailHolderHelper.ViewHolderHelper1(mContext,
                            mDataVisible, (ViewPagerViewHolder) holder, infoItemBean, listener);
                    break;

                //(只有四个按钮)
                case HomeOneViewType.bottom_viewType:
                    DetailHolderHelper.ViewHolderHelper2
                            ((TagViewHolder) holder, infoItemBean, listener);
                    break;

                //(四张小图,24号文字)
                case HomeOneViewType.common_viewType:
                    DetailHolderHelper.ViewHolderHelper3(mContext,
                            mDataVisible, (CommonViewHolder) holder, infoItemBean,
                            mLayoutItemList.get(position));
                    break;

                //(四张小图,24文字,22号文字)
                case HomeOneViewType.common_viewType1:
                    DetailHolderHelper.ViewHolderHelper4(mContext,
                ...
                //(四张小图,24文字,22号文字,时长)
                case HomeOneViewType.persontag_viewType:
                    DetailHolderHelper.ViewHolderHelper5(mContext, mDataVisible,
                            (SpaceViewHolder) holder, infoItemBean, mLayoutItemList.get(position), listener);
                    break;

                //(一张大图,24文字)
                case HomeOneViewType.imagview_viewType:
                    DetailHolderHelper.ViewHolderHelper6(mContext, mDataVisible,
                            (CommonViewHolder) holder, infoItemBean, mLayoutItemList.get(position));
                    break;

                //(一张大图,24文字,四张小图,24文字,24号文字)
                case HomeOneViewType.commonList_viewType:
                    DetailHolderHelper.ViewHolderHelper7(mContext,
                            mDataVisible, (CommonListHolder) holder, infoItemBean,
                            mLayoutItemList.get(position), listener);
                    break;
                ...
                //(三张长图,24文字)
                case HomeOneViewType.space_viewType:
                    DetailHolderHelper.ViewHolderHelper8(mContext, mDataVisible,
                            (CommonListView) holder, infoItemBean, mLayoutItemList.get(position), listener);
                    break;

                //(四张小图,24文字,24号文字)
                case HomeOneViewType.foot_viewType:
                    DetailHolderHelper.ViewHolderHelper9(mContext,
