...
public class HomeTwoLiveSceneFragment extends BaseFragment {
    ...
    public static HomeTwoLiveSceneFragment newInstance(Bundle bundle) {
        HomeTwoLiveSceneFragment fragment = new HomeTwoLiveSceneFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        root = inflater.inflate(R.layout.home_two_live_item_scene_fragment, container, false);
    ...
    private void setUI() {
        mToRefreshLayout = (PullToRefreshLayout) root.findViewById(R.id.home_two_live_item_scene_refresh);
        gridView = (GridView) root.findViewById(R.id.home_two_live_item_scene_grid);
    ...
    public void setData(Bundle bundle) {
        ...
        if (infoBlockBean != null) {
            ...
            if (contentsList != null && contentsList.size() > 0) {
                channelID = contentsList.get(0).getContentId();
                getLiveSceneData(channelID);
            } else {
                handleErrorResponse();
            }
        } else {
...
//                        ToastUtils.show(context, "施主,没有内容了,请回头吧");
     ...
     * @param channelID
     */
    private void getLiveSceneData(String channelID) {
        ...
        HttpRequest.getInstance().excute("getListProgramData", new Object[]{getListProgramDataUrl,
            ...
            public void onSuccess(ScreeningDataBean screeningDataBean) {
                Lg.i(TAG, "转播页面 - 现场下的数据：" + screeningDataBean.toString());
                handleLoadRecommendListSuccessResponse(screeningDataBean);
            }

            @Override
            public void onError(String error) {
                Lg.i(TAG, "转播页面 - 现场下的数据：getHomeNavContent : error--> " + error);
                handleErrorResponse();
            }
        }});
    }
     ...
     * 获取现场节目直播状态数据
     */
    private void getLiveSceneStatusData() {
        ...
        HttpRequest.getInstance().excute("getLiveStatus", new Object[]{url,
            ...
            public void onSuccess(LiveStatusBean data) {
                Lg.i(TAG, "转播页面 - 现场下的节目状态数据：" + data.toString());
                handleLoadLiveStatusBeanSuccessResponse(data);
            }

            @Override
            public void onError(String error) {
                Lg.i(TAG, "转播页面 - 现场下的节目状态数据：getLiveStatus : error--> " + error);
                handleErrorResponse();
            }
        }});
    }
