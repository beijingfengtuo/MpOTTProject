...
class StickyListHeadersAdapterWrapper extends BaseAdapter implements StickyListHeadersAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (viewType == headerViewType) {
			...
			toPosition = translateListViewPosition(position);
			convertView = delegate.getView(toPosition, convertView, parent);
		}
		toView = convertView;
		return convertView;
	}
	...
	/** 获取控件是否到底部的方法 **/
	public boolean getToPosition() {
		int position = toPosition + 1;
		if (position == delegate.getCount()) {
			return true;
		}
		return false;
	}
