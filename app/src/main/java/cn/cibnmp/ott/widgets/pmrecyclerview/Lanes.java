...
import cn.cibnmp.ott.widgets.pmrecyclerview.TwoWayLayoutManager.Direction;
import cn.cibnmp.ott.widgets.pmrecyclerview.TwoWayLayoutManager.Orientation;
...
public class Lanes {
    ...
    private final double mLaneSize;
    ...
    public Lanes(BaseLayoutManager layout, Orientation orientation, Rect[] lanes, double laneSize) {
        mLayout = layout;
    ...
    public Lanes(BaseLayoutManager layout, int laneCount) {
        ...
        for (int i = 0; i < laneCount; i++) {
            ...
            final int r = (int) (mIsVertical ? l + mLaneSize : l);
            final int b = (int) (mIsVertical ? t : t + mLaneSize);

            mLanes[i].set(l, t, r, b);
        }
    }

    public static double calculateLaneSize(BaseLayoutManager layout, int laneCount) {
        if (layout.isVertical()) {
            final int paddingLeft = layout.getPaddingLeft();
            final int paddingRight = layout.getPaddingRight();
            final int width = layout.getWidth() - paddingLeft - paddingRight;
//            Log.d("calculateLaneSize", "calculateLaneSize : " +
//                    ((double) width / laneCount) + " , " + width + " , paddingLeft : " + layout.getPaddingLeft());
            return (double) width / laneCount;
        } else {
            final int paddingTop = layout.getPaddingTop();
            final int paddingBottom = layout.getPaddingBottom();
            final int height = layout.getHeight() - paddingTop - paddingBottom;
            return (double) height / laneCount;
        }
    }
