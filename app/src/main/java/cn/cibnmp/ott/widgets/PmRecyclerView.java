...
import cn.cibnmp.ott.widgets.pmrecyclerview.BaseLayoutManager;
import cn.cibnmp.ott.widgets.pmrecyclerview.TwoWayLayoutManager;

/**
 * Created by yanjing on 2017/9/21.
 */

public class PmRecyclerView extends RecyclerView implements Pullable {
    private static final String LOGTAG = PmRecyclerView.class.getSimpleName() + ":::";
    ...
    private PmRecyclerView.OnItemListener mOnItemListener;
    private PmRecyclerView.OnInBorderKeyEventListener mOnInBorderKeyEventListener;
    private PmRecyclerView.OnLoadMoreListener mOnLoadMoreListener;
    private boolean mHasMore = true;
    private boolean mLoadingMore = false;

    private PmRecyclerView.ItemListener mItemListener;
    ...
    public PmRecyclerView(Context context) {
        this(context, null);
    }

    public PmRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PmRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        mItemListener = new PmRecyclerView.ItemListener() {
            /**
             * 子控件的点击事件
             * @param itemView
             */
            ...
            public void onClick(View itemView) {
                if (null != mOnItemListener) {
                    mOnItemListener.onItemClick(PmRecyclerView.this, itemView, getChildLayoutPosition(itemView));
                }
            }
            ...
            public void onFocusChange(View itemView, boolean hasFocus) {
                ...
                if (null != itemView) {
                    ...
                    if (hasFocus) {
                        ...
                        if (null != mOnItemListener)
                            mOnItemListener.onItemSelected(PmRecyclerView.this, itemView, position);
                    } else {
                        mOldSelectedPosition = position;
                        if (null != mOnItemListener)
                            mOnItemListener.onItemPreSelected(PmRecyclerView.this, itemView, position);
                    }
                }
            }
        };
    ...
    private void loadLayoutManagerFromName(Context context, AttributeSet attrs, String name) {
        try {
            ...
            if (dotIndex == -1) {
                name = "cn.cibnmp.ott.widgets.pmrecyclerview.widget." + name;
            } else if (dotIndex == 0) {
...
//        Log.i(LOGTAG, "requestChildFocus: "+child);
    ...
    private boolean handleOnKey(int keyCode, KeyEvent event) {
        ...
        if (direction == -1) {
            ...
            View newFocusedView = getLayoutManager().onInterceptFocusSearch(getFocusedChild(), direction);
            if (newFocusedView == null)
                newFocusedView = findNextFocus(direction);
            if (null != newFocusedView) {
                //newFocusedView.requestFocus();
                handleNextFocusView(newFocusedView, direction);
            } else {
                handleNextFocusView(null, direction);
            }
        }
        return true;
    }
    ...
    public interface FocusHandler {
        //是否拦截，若返回true，则拦截
        boolean handle(View focus, int direction);
    }

    private PmRecyclerView.FocusHandler focusHandler;

    public void setFocusHandler(PmRecyclerView.FocusHandler focusHandler) {
        this.focusHandler = focusHandler;
    }


    /**
     * wanqi,首页特殊焦点情况处理，recyclerview里内嵌recyclerview的焦点问题
     *
     * @param focus
     * @param direction
     */
    private void handleNextFocusView(View focus, int direction) {

        if (focusHandler != null) {
            if (focusHandler.handle(focus, direction))
                return;
        }
    ...
    private boolean hasInBorder(int direction) {
        ...
        if (null != view) {
            ...
            switch (direction) {
                case FOCUS_DOWN:
//                    Lg.d("outRect : " + outRect.toString() + " , getHeight() - view.getBottom() : " + (getHeight() - view.getBottom()));
                ...
                case FOCUS_RIGHT:
                    if (isVertical()) {
                        result = getWidth() - view.getRight() <= getPaddingRight() + lp.rightMargin + outRect.right + DisplayUtils.getValue(10);
                    } else {
                        result = getWidth() - view.getRight() <= getPaddingRight() + lp.rightMargin + outRect.right;
                    }
//                    Log.d("wanqi", "getWidth() - view.getRight() : " + (getWidth() - view.getRight()) +
//                            " ,  getPaddingRight() + lp.rightMargin + outRect.right : " + (getPaddingRight() + lp.rightMargin + outRect.right));
...
//        Log.i(LOGTAG, "onFocusChanged..." + gainFocus + " ,direction="+direction + " ,mOldSelectedPosition="+mOldSelectedPosition);
     ...
     * 清除menu 状态
     *
     * @param pos menu 自己记录的状态
     */
    public void cleanMenuPos(int pos) {
        if (mIsMenu && pos == mOldSelectedPosition) {
            ViewHolder holder = findViewHolderForLayoutPosition(mOldSelectedPosition);
            if (null != holder) {
...
//        Log.i(LOGTAG, "isInTouchMode...");
    ...
    protected Parcelable onSaveInstanceState() {
        PmRecyclerView.SavedState savedState = new PmRecyclerView.SavedState(super.onSaveInstanceState());
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        try {
            if (null != state) {
                if (state instanceof PmRecyclerView.SavedState) {
                    PmRecyclerView.SavedState savedState = (PmRecyclerView.SavedState) state;
    ...
    protected static class SavedState implements Parcelable {
        protected static final PmRecyclerView.SavedState EMPTY_STATE = new PmRecyclerView.SavedState();
        ...
        public static final Creator<PmRecyclerView.SavedState> CREATOR
                = new Creator<PmRecyclerView.SavedState>() {
            @Override
            public PmRecyclerView.SavedState createFromParcel(Parcel in) {
                return new PmRecyclerView.SavedState(in);
            }

            @Override
            public PmRecyclerView.SavedState[] newArray(int size) {
                return new PmRecyclerView.SavedState[size];
            }
        };
    }

    public void setOnItemListener(PmRecyclerView.OnItemListener onItemListener) {
        mOnItemListener = onItemListener;
    }

    public void setOnInBorderKeyEventListener(PmRecyclerView.OnInBorderKeyEventListener onInBorderKeyEventListener) {
        mOnInBorderKeyEventListener = onInBorderKeyEventListener;
    }

    public void setOnLoadMoreListener(PmRecyclerView.OnLoadMoreListener onLoadMoreListener) {
    ...
    public interface OnItemListener {
        void onItemPreSelected(PmRecyclerView parent, View itemView, int position);

        void onItemSelected(PmRecyclerView parent, View itemView, int position);

        void onReviseFocusFollow(PmRecyclerView parent, View itemView, int position);

        void onItemClick(PmRecyclerView parent, View itemView, int position);
    }
